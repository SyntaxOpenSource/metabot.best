    ...
    fetch(`./${arguments.location}/${user}.json`).then(response => response.json())
        .then(json => {
            labels.forEach(element => {
                if (element.dataset.btnItem) {
                    if (localStorage.getItem('user') === element.dataset.btnItem) {
                        element.setAttribute('class', '');
                        element.setAttribute('class', 'select');
                    }

                    element.addEventListener('click', () => {
                        localStorage.setItem('user', element.dataset.btnItem); // update user
...
Translater({
    ...
    location: 'languages', // Folder name where your language files are located
});